...
export default function Toolbar(props) {
    const {workflow, edges, nodes, sendError} = useWorkflowContext();
    ...
    const downloadWorkflowJson = (name) => {
        ...
        if (!validate(cleanObject(cleanedWorkflow))){ // If violates Schema
            console.log(validate.errors) 
            const errorMsg = validate.errors.map((error, i) => {
                // Extract just the final part of the field path (like "FunctionName")
    ...
    return(
        <div id="toolbar" style={{ width: '100vw', height: '5vh'}}>
            ...
            <Popup enabled={uploadPopupEnabled} setEnabled={() => setUploadPopupEnabled()} >
                <UploadWorkflow setChanged={() => setChanged()} setLayout={() => props.setLayout()}createEdge={ props.createEdge } createNode={props.createNode} workflow_template={props.workflow_template}/>
